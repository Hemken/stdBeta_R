...
QuantPsyc::lm.beta(fit) # warning given
lm.beta::lm.beta(fit)
lsr::standardCoefs(fit) # agrees with lm.beta::lm.beta, but no intercept
arm::standardize(fit, standardize.y=TRUE)
```

The `QuantPsyc` function is implemented in a way that breaks on
higher order terms.  The `lm.beta` and `lsr` functions apply
